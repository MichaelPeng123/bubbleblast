...
class TrailBubble {
  ...
  update(deltaTime) {
    ...
    if (this.mesh.material) {
      this.mesh.material.opacity = 0.6 * (1 - this.age / this.lifetime); // Adjusted from 0.7
      
      // Slightly grow the bubble as it ages
      const scale = 1 + (this.age / this.lifetime) * 0.3; // Reduced growth (was 0.5)
...
export class BulletSystem {
  constructor(scene, levelManager) {
    ...
    this.trailSpawnRate = 0.05; // Time between trail bubbles
    this.lastTrailTime = 0;
    this.trailBubbleSize = 0.015; // Much smaller trail bubbles (was 0.05)
  ...
  createBullet(position, direction) {
    ...
    // Reset clock for this new bullet
    this.lastTrailTime = 0;
  }
  ...
  update(roomBounds) {
    ...
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      if (this.lastTrailTime >= this.trailSpawnRate) {
        // Create a trail bubble at current position
